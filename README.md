...
- **URL deep linking:** `?article=<id>` query parametresi ile analiz modalı doğrudan açılabilir
- **Terminoloji:** UI metinlerinde “makale” → “analiz” (TR/EN sözlük, mobil nav, modal boş içerik mesajı)
...
│   │   ├── articles.ts         # Analiz yardımcıları (yazar profili, LinkedIn URL, sabit rastgele menü seçimi)
...
- **MegaMenus (`AnalysesMenu`):** Sayfa yüklemesine sabit rastgele 1 öne çıkan + 3 yan analiz; görselsiz kart meta, yazar portresi ve `?article=` deep link
...
### Kaynak / Analiz Bileşenleri
- **ArticleCardHeader:** Paylaşımlı kart/modal/mega menü üst bölümü — etiketler, vurgulu alıntı, yazar bilgisi, tema renkli ayırıcı (`CARD_THEMES`, `articleTypography`)
- **ArticlesHero:** Analizler sayfası hero — `analizler-hero.png` arka plan, koyu slate gradient overlay, beyaz başlık tipografisi
- **ArticleModal:** Portal tabanlı analiz detay modalı — `ArticleCardHeader` + başlık/meta + Markdown gövde; ESC, focus trap
...
|----------|-------|-------------------------|
| v0.54.0 | 2026-09 | **ConnectED içerik ve SSS genişletmesi:** Hero başlık/alt metin ve Development pillar açıklaması yeniden yazıldı; "Teknik yetkinlik geliştirme başlıklarımız" → "ConnectED Teknoloji ve Uzmanlık Alanları" başlığıyla ve büyük harfli kategori adlarıyla güncellendi; yeni **Sektör Vitrini** bölümü (`industryShowcase`) eklendi; "Doğru Yeteneği İş Ortaklarımızla Buluşturuyoruz" değer bölümü metni revize edildi; SSS'ye 4 yeni soru eklendi (seçme süreci süresi, eğitim formatı, eğitmenler, çalışma modeli, sektörler, başarı oranı); SSS akordeonu tek seferde bir soru açık kalacak şekilde state ile kontrol edilir hale getirildi (`useState`); alıntı kartlarında tırnak işareti stili sadeleştirildi. |
