...
- **ArticlesHero:** Açık tonlu toplantı fotoğrafı (`analizler-hero.png`); slider görseli (`analizler.png`) ayrı tutulur; başlık stili diğer Bilgi Merkezi sayfalarıyla uyumlu (`text-corporate-dark font-black`)
- **Görselsiz kart tasarımı:** Liste kartlarında kapak görseli yok; etiketler, vurgulu alıntı, yazar bilgisi (ünvan + Teknoloji Servisleri), renkli ayırıcı çizgi, başlık, özet ve okuma süresi
...
- **7 makale** — Yapay Zeka, Otomasyon, Güvenlik, Yönetişim, Altyapı; `cardTags`, `cardQuote`, `cardTheme` alanları ile kart meta verisi
...
- **URL deep linking:** `?article=<id>` query parametresi ile makale modalı doğrudan açılabilir
- **İçerik düzenlemesi:** TR makale metinlerinde yazım, dilbilgisi ve devrik cümle düzeltmeleri
...
│   │   ├── headers/            # Sayfa hero görselleri (analizler.png slider, analizler-hero.png sayfa hero)
...
│   │   ├── resources/          # ArticleCardHeader, ArticlesHero, ArticleModal, CaseStudyCard
...
│   │   ├── articles.ts         # Makale yardımcıları (yazar profili, LinkedIn URL, son N makale seçimi)
...
- **ArticleCardHeader:** Paylaşımlı kart/modal üst bölümü — etiketler, vurgulu alıntı, yazar bilgisi, tema renkli ayırıcı (`CARD_THEMES`, `articleTypography`)
- **ArticlesHero:** Analizler sayfası hero — `analizler-hero.png` arka plan, açık overlay, standart kurumsal başlık tipografisi
- **ArticleModal:** Portal tabanlı makale detay modalı — `ArticleCardHeader` + başlık/meta + Markdown gövde; ESC, focus trap
...
| `src/lib/articles.ts` | Makale yardımcıları — yazar profili, LinkedIn URL, son N makaleden rastgele seçim |
