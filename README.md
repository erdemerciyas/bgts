...
| CSS Utility | tailwind-merge + clsx | — |
| Form Validasyonu | Zod | ^4.3 |
...
- **Mega Menu Navigasyon:** Hizmetler, Ürünler, Sektörler ve Bilgi Merkezi için tam genişlikte mega menüler; Bilgi Merkezi sol panelinde EBINTEC etkinlik kartı (`ResourcesMenuLeftPanel.tsx`); Analizler kartında son 5 makaleden rastgele **tek** makale (kapak özeti, yazar portresi, `?article=` deep link)
...
### Makaleler / Analizler
- **Analizler sayfası:** `/bilgi-merkezi/analizler` (TR) / `/resources/analyses` (EN) — kurumsal blog ve teknik makaleler
- **ArticlesHero:** Kurumsal editorial hero — grid arka plan, gradient dekorasyon, çift kolaj görsel düzeni
- **7 makale** — Yapay Zeka, Otomasyon, Güvenlik, Yönetişim, Altyapı kategorilerinde; tam Markdown içerik ve kapak görselleri
- **Yazar portreleri:** `public/images/articles/writer/` (Alper Önsoy, Erdoğan Bilici, Sinan Demirci); eşleşmeyen yazarlar için BGTS logosu fallback (`src/lib/articles.ts` → `getAuthorAvatar`)
- Kategori bazlı renk kodlu badge sistemi (TR/EN kategori eşleşmesi)
- **Modal detay görünümü** (`ArticleModal.tsx`) — Markdown render, yazar avatarı, klavye erişilebilirlik (ESC, focus trap), viewport'a sığan flex layout; kapak görseli, başlık/meta ve makale gövdesi tek scroll alanında
- **URL deep linking:** `?article=<id>` query parametresi ile makale modalı doğrudan açılabilir; mega menü ve kart tıklamaları URL'i günceller (`ArticlesClient.tsx`, Suspense sınırı)
- **Bilgi Merkezi nav:** Son 5 makaleden rastgele 1 tanesi Analizler kartında gösterilir (başlık + özet + yazar fotoğrafı)
...
│   │   │   └── writer/         # Yazar portreleri (BGTS logo fallback)
...
│   │   │   │   ├── infographics/
│   │   │   │   ├── success-stories/
│   │   │   │   └── analyses/       # Analizler sayfası (ArticlesClient, ArticlesHero, ArticleModal)
│   │   │   │
│   │   │   ├── layout.tsx      # Lang layout (Header, Footer, GA, CookieConsent)
│   │   │   ├── page.tsx        # Ana sayfa
│   │   │   ├── error.tsx       # Global hata sayfası
│   │   │   ├── not-found.tsx   # 404 sayfası (metadata)
│   │   │   └── NotFoundClient.tsx # Locale-aware 404 UI
│   │   │
│   │   ├── favicon.ico         # Favicon
...
│   │   │   ├── header/         # MegaMenus.tsx, ResourcesMenuLeftPanel.tsx, data.ts
│   │   │   └── search/         # SearchOverlay.tsx, data.ts
│   │   ├── resources/          # ArticlesHero, ArticleModal, CaseStudyCard
...
│   │   ├── articles.ts         # Makale yardımcıları (son 5'ten rastgele seçim, yazar avatarı)
...
| `/resources/analyses` | `/bilgi-merkezi/analizler` |
...
| `/tr/urunler/praxilla` | `/tr/en/products/praxilla` | Praxilla |
...
| `/tr/bilgi-merkezi/analizler` | `/tr/en/resources/analyses` | Analizler |
...
- **ResourcesMenuLeftPanel:** Bilgi Merkezi mega menüsü sol paneli — EBINTEC etkinlik kartı
- **MegaMenus (Analizler kartı):** Son 5 makaleden rastgele 1 makale; özet, yazar portresi ve `?article=` deep link
...
### Kaynak / Makale Bileşenleri
- **ArticlesHero:** Makaleler sayfası kurumsal hero alanı (editorial kolaj, Framer Motion fade-up)
- **ArticleModal:** Portal tabanlı makale detay modalı — kapak banner, yazar avatarı, meta satırı, Markdown gövde; tüm içerik tek scroll konteynerinde, scroll lock
- **CaseStudyCard:** Başarı hikayesi kart bileşeni
...
- **CaseStudyCard:** Vaka çalışması kart bileşeni
...
| `src/lib/articles.ts` | Makale yardımcıları — son N makaleden rastgele seçim, yazar avatar eşleştirmesi |
| `src/data/articles.tr.ts` | Analizler — Türkçe (başlık, özet, body, kategori, yazar, kapak) |
